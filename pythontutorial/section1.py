...
class EmployeeDatabase(object):
    def __init__(self, data_csv_table_path: str):
        self._employee_dataframe = self.__create_employee_dataframe(data_csv_table_path = data_csv_table_path)
        self.__get_all_employee_info()
        self.__generate_employee_objects(employee_dataframe=self._employee_dataframe)
    def __create_employee_dataframe(self, data_csv_table_path):
        return pd.read_csv(data_csv_table_path)
    def __get_all_employee_info(self): # the instance attribute is only produced when this method is invoked
        self._employee_names = list(self._employee_dataframe[NAME])
        self._employee_ids = list(self._employee_dataframe[ID])
        self._employee_age = list(date.today().year - self._employee_dataframe[YOB])
    ...
    def __generate_employee_objects(self, employee_dataframe):
        ...
        for row in employee_dataframe.values: # for each ndarray of object
            ...
            employee = Employee(attributes_list=employee_attributes_list, values_list=employee_values_list)
